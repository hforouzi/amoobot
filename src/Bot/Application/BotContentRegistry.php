...
final class BotContentRegistry
{
    ...
    private function persianMessages(): array
    {
        return [
            ...
            'payment.manual_card.instructions' => ['title' => 'Manual card payment instructions', 'body' => "کد پیگیری سفارش شما:\n{{ order.trackingCode }}\n\nشناسه سرویس: {{ order.serviceId }}\nمبلغ پایه: {{ order.baseAmount }} تومان\nتخفیف سراسری: {{ order.globalDiscount }} تومان\nکد تخفیف: {{ order.discountCode }} ({{ order.discountAmount }} تومان)\nمبلغ نهایی: {{ order.finalAmount }} تومان\n\nشماره کارت:\n{{ payment.cardNumber }}\n\nبه نام:\n{{ payment.cardHolder }}\n\n{{ payment.extraInstructions }}\n\nبرای ارسال رسید روی «{{ button.confirmAndSendReceipt }}» بزنید.", 'category' => 'payments', 'variables' => ['order.trackingCode', 'order.serviceId', 'order.baseAmount', 'order.globalDiscount', 'order.discountCode', 'order.discountAmount', 'order.finalAmount', 'payment.cardNumber', 'payment.cardHolder', 'payment.extraInstructions', 'button.confirmAndSendReceipt']],
            'payment.manual_card.plan_instructions' => ['title' => 'Manual card plan payment instructions', 'body' => "کد پیگیری سفارش شما:\n{{ order.trackingCode }}\n\nپلن: {{ plan.title }}\nمبلغ پایه: {{ order.baseAmount }} تومان\nتخفیف سراسری: {{ order.globalDiscount }} تومان\nکد تخفیف: {{ order.discountCode }} ({{ order.discountAmount }} تومان)\nمبلغ نهایی: {{ order.finalAmount }} تومان\n\nشماره کارت:\n{{ payment.cardNumber }}\n\nبه نام:\n{{ payment.cardHolder }}\n\n{{ payment.extraInstructions }}\n\nبرای ارسال رسید روی «{{ button.confirmAndSendReceipt }}» بزنید.", 'category' => 'payments', 'variables' => ['order.trackingCode', 'plan.title', 'order.baseAmount', 'order.globalDiscount', 'order.discountCode', 'order.discountAmount', 'order.finalAmount', 'payment.cardNumber', 'payment.cardHolder', 'payment.extraInstructions', 'button.confirmAndSendReceipt']],
            'payment.manual_card.custom_order_instructions' => ['title' => 'Manual card custom order payment instructions', 'body' => "کد پیگیری سفارش شما:\n{{ order.trackingCode }}\n\nپلن: {{ plan.title }}\nنام کاربری: {{ order.accountName }}\nحجم: {{ order.trafficGb }} گیگ\nمدت: {{ order.duration }}\nمبلغ پایه: {{ order.baseAmount }} تومان\nتخفیف سراسری: {{ order.globalDiscount }} تومان\nکد تخفیف: {{ order.discountCode }} ({{ order.discountAmount }} تومان)\nمبلغ نهایی: {{ order.finalAmount }} تومان\n\nشماره کارت:\n{{ payment.cardNumber }}\n\nبه نام:\n{{ payment.cardHolder }}\n\n{{ payment.extraInstructions }}\n\nبرای ارسال رسید روی «{{ button.confirmAndSendReceipt }}» بزنید.", 'category' => 'payments', 'variables' => ['order.trackingCode', 'plan.title', 'order.accountName', 'order.trafficGb', 'order.duration', 'order.baseAmount', 'order.globalDiscount', 'order.discountCode', 'order.discountAmount', 'order.finalAmount', 'payment.cardNumber', 'payment.cardHolder', 'payment.extraInstructions', 'button.confirmAndSendReceipt']],
            'payment.manual_card.incomplete_instructions' => ['title' => 'Manual card incomplete payment instructions', 'body' => "سفارش ناتمام شما:\nکد پیگیری: {{ order.trackingCode }}\nمبلغ: {{ order.finalAmount }} تومان\n\nشماره کارت:\n{{ payment.cardNumber }}\n\nبه نام:\n{{ payment.cardHolder }}\n\n{{ payment.extraInstructions }}\n\nبرای ارسال رسید روی «{{ button.confirmAndSendReceipt }}» بزنید.", 'category' => 'payments', 'variables' => ['order.trackingCode', 'order.finalAmount', 'payment.cardNumber', 'payment.cardHolder', 'payment.extraInstructions', 'button.confirmAndSendReceipt']],
    ...
    private function persianButtons(): array
    {
        return [
            ...
            'button.common.back_to_main_menu' => ['label' => 'بازگشت به منوی اصلی', 'buttonType' => 'inline', 'category' => 'common'],
            ...
            'button.admin.management' => ['label' => 'مدیریت', 'buttonType' => 'inline', 'category' => 'admin'],
            'button.admin.store_management' => ['label' => 'مدیریت فروشگاه', 'buttonType' => 'inline', 'category' => 'admin'],
            'button.admin.payments' => ['label' => 'مدیریت پرداخت‌ها', 'buttonType' => 'inline', 'category' => 'admin'],
            'button.admin.services' => ['label' => 'مدیریت سرویس‌ها', 'buttonType' => 'inline', 'category' => 'admin'],
            'button.admin.users' => ['label' => 'مدیریت کاربران', 'buttonType' => 'inline', 'category' => 'admin'],
            'button.admin.store.settings' => ['label' => 'تنظیمات فروشگاه', 'buttonType' => 'inline', 'category' => 'admin_store'],
            'button.admin.store.plans' => ['label' => 'پلن‌ها', 'buttonType' => 'inline', 'category' => 'admin_store'],
            'button.admin.store.discounts' => ['label' => 'کدهای تخفیف', 'buttonType' => 'inline', 'category' => 'admin_store'],
            'button.admin.store.payment_methods' => ['label' => 'روش‌های پرداخت', 'buttonType' => 'inline', 'category' => 'admin_store'],
            'button.admin.store.payment_gateways' => ['label' => 'درگاه‌های پرداخت', 'buttonType' => 'inline', 'category' => 'admin_store'],
            'button.admin.store.orders' => ['label' => 'سفارش‌ها', 'buttonType' => 'inline', 'category' => 'admin_store'],
            'button.admin.back_to_management' => ['label' => 'بازگشت به مدیریت', 'buttonType' => 'inline', 'category' => 'admin'],
            ...
            'button.confirm_and_send_receipt' => ['label' => '✅ تایید و ارسال رسید', 'buttonType' => 'inline', 'category' => 'payment'],
    ...
    private function englishButtons(): array
    {
        ...
        $labels = [
            ...
            'button.common.back_to_main_menu' => 'Back to main menu',
            'button.common.cancel' => '❌ Cancel',
            'button.common.confirm' => '✅ Confirm',
            'button.common.close' => '❌ Close',
            'button.admin.management' => 'Admin',
            'button.admin.store_management' => 'Store management',
            'button.admin.payments' => 'Payment management',
            'button.admin.services' => 'Service management',
            'button.admin.users' => 'User management',
            'button.admin.store.settings' => 'Store settings',
            'button.admin.store.plans' => 'Plans',
            'button.admin.store.discounts' => 'Discount codes',
            'button.admin.store.payment_methods' => 'Payment methods',
            'button.admin.store.payment_gateways' => 'Payment gateways',
            'button.admin.store.orders' => 'Orders',
            'button.admin.back_to_management' => 'Back to admin',
            ...
            'button.confirm_and_send_receipt' => '✅ Confirm and send receipt',
